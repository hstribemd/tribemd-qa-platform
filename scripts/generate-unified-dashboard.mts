...
// Importa o tipo Defect sem o DefectCollector (singleton por locale)
type Defect = Awaited<ReturnType<import('../src/core/defect-collector.js').DefectCollector['readAll']>>[number];

const ALL_LOCALES = ['br', 'us', 'es'];
...
const LOCALE_LABEL: Record<string, string> = { br: 'BR', us: 'US', es: 'ES' };
...
async function readDefects(): Promise<Defect[]> {
  const all: Defect[] = [];
  for (const locale of ALL_LOCALES) {
    const dir = path.resolve('evidencias', locale);
    try {
      const entries = await fs.readdir(dir);
      for (const e of entries) {
        try {
          const raw = await fs.readFile(path.join(dir, e, 'defect.json'), 'utf-8');
          all.push(JSON.parse(raw) as Defect);
        } catch { /* pasta sem defect.json */ }
      }
    } catch { /* locale sem evidencias */ }
  }
  return all.sort((a, b) => SEV_ORDER[a.severity] - SEV_ORDER[b.severity] || a.id.localeCompare(b.id));
}

async function readTotalPages(): Promise<number> {
  let total = 0;
  for (const locale of ALL_LOCALES) {
    try {
      const meta = JSON.parse(await fs.readFile(path.resolve('reports', locale, 'run-meta.json'), 'utf-8'));
      total += meta.pages ?? 0;
    } catch { /* sem meta */ }
  }
  return total;
...
async function main(): Promise<void> {
  const defects = await readDefects();
  const totalPages = await readTotalPages();
  const counts: Record<Severity, number> = { Critical: 0, High: 0, Medium: 0, Low: 0 };
  const localeCounts: Record<string, number> = { br: 0, us: 0, es: 0 };
  defects.forEach((d) => {
    counts[d.severity]++;
    if (d.locale) localeCounts[d.locale.toLowerCase()] = (localeCounts[d.locale.toLowerCase()] ?? 0) + 1;
  });
  ...
  const summaryLines = [
    `QA TribeMD — BR · US · ES — ${new Date().toLocaleDateString('pt-BR')}`,
    totalPages > 0
      ? `${totalPages} páginas verificadas · ${defects.length} defeito(s) encontrado(s)`
  ...
  const reportFilename = `tribemd-qa-${new Date().toISOString().slice(0, 10)}.html`;

  const activeLocales = ALL_LOCALES.filter((l) => localeCounts[l] > 0);
  ...
  for (const d of defects) {
    ...
    const dLocale = (d.locale ?? '').toLowerCase();

    cards.push(`
<article class="card" data-sev="${d.severity}" data-locale="${dLocale}" style="border-left:4px solid ${SEV_COLOR[d.severity]}">
  ...
  <div class="card-body" style="background:${SEV_BG[d.severity]}08">
    ...
    <div class="jira-section">
      ...
      <div class="ev-grid">
        ...
        ${!shownImg ? `<div class="no-ev">Sem evidência de mídia registrada</div>` : ''}
      </div>
    </div>
  </div>
</article>`);
  ...
  // Locale filter buttons (só mostra se mais de 1 locale tem dados)
  const localeFilterRow = activeLocales.length > 1
    ? `<div class="tb-sep"></div>
       <button class="tb-btn on" data-l="all" onclick="filtLocale(this,'all')">Todos os locales</button>
       ${ALL_LOCALES.filter(l => localeCounts[l] > 0).map(l =>
         `<button class="tb-btn" data-l="${l}" onclick="filtLocale(this,'${l}')">${LOCALE_LABEL[l]} (${localeCounts[l]})</button>`
       ).join('')}`
    : '';
...
<title>QA Report — TribeMD BR · US · ES</title>
<style>
...
.locale-tags{display:flex;gap:4px}
.locale-tag{background:#1f6feb;color:#fff;font-size:11px;font-weight:700;padding:2px 8px;border-radius:4px;letter-spacing:.5px}
...
<div class="topbar">
  <div class="brand">
    ...
    <div class="locale-tags">
      ${ALL_LOCALES.map(l => `<span class="locale-tag">${LOCALE_LABEL[l]}</span>`).join('')}
    </div>
  </div>
  <div class="report-meta">Gerado em ${new Date().toLocaleString('pt-BR')}</div>
  <div class="action-btns">
    <button class="action-btn" onclick="copySummary(this)">Copiar resumo</button>
    <button class="action-btn" onclick="window.print()">Imprimir / PDF</button>
    <button class="action-btn primary" onclick="downloadReport()">Baixar relatório</button>
  </div>
</div>
...
<div class="stats-bar">
  ${totalPages > 0 ? `<div class="stat-block"><div class="stat-n">${totalPages}</div><div class="stat-l">Páginas verificadas</div></div><div class="vd"></div>` : ''}
...
<div class="toolbar">
  <input class="search-input" type="search" placeholder="Buscar por URL, título, erro…" oninput="applyFilters()" aria-label="Buscar defeitos">
  <div class="tb-sep"></div>
  <button class="tb-btn on" data-f="all" onclick="filtSev(this,'all')">Todos (${defects.length})</button>
  ${counts.Critical > 0 ? `<button class="tb-btn" data-f="Critical" onclick="filtSev(this,'Critical')">Crítico (${counts.Critical})</button>` : ''}
  ${counts.High > 0 ? `<button class="tb-btn" data-f="High" onclick="filtSev(this,'High')">Alto (${counts.High})</button>` : ''}
  ${counts.Medium > 0 ? `<button class="tb-btn" data-f="Medium" onclick="filtSev(this,'Medium')">Médio (${counts.Medium})</button>` : ''}
  ${counts.Low > 0 ? `<button class="tb-btn" data-f="Low" onclick="filtSev(this,'Low')">Baixo (${counts.Low})</button>` : ''}
  ${localeFilterRow}
  <div class="tb-sep"></div>
...
function applyFilters() {
  const sev = document.querySelector('[data-f].on')?.dataset.f || 'all';
  const loc = document.querySelector('[data-l].on')?.dataset.l || 'all';
  const term = (document.querySelector('.search-input').value || '').toLowerCase();
  document.querySelectorAll('.card').forEach(c => {
    const ok = (sev === 'all' || c.dataset.sev === sev)
            && (loc === 'all' || c.dataset.locale === loc)
            && (!term || c.textContent.toLowerCase().includes(term));
    c.style.display = ok ? '' : 'none';
  });
}
function filtSev(btn, sev) {
  document.querySelectorAll('[data-f]').forEach(b => b.classList.remove('on'));
  btn.classList.add('on');
  applyFilters();
}
function filtLocale(btn, loc) {
  document.querySelectorAll('[data-l]').forEach(b => b.classList.remove('on'));
  btn.classList.add('on');
  applyFilters();
}
function copyUrl(btn) {
  const url = btn.dataset.url;
  const done = () => { btn.classList.add('ok'); btn.textContent = '✓'; setTimeout(() => { btn.textContent = '⧉'; btn.classList.remove('ok'); }, 1500); };
  if (navigator.clipboard) navigator.clipboard.writeText(url).then(done);
  else { fallbackCopy(url); done(); }
}
function copySummary(btn) {
  const done = () => { const p = btn.textContent; btn.textContent = 'Copiado!'; setTimeout(() => btn.textContent = p, 2000); };
...
function downloadReport() {
  ...
  const out = path.resolve('reports', 'index.html');
  await fs.writeFile(out, html, 'utf-8');
  console.log(`Dashboard unificado gerado em ${out} (${defects.length} defeito(s) — BR/US/ES)`);
}
